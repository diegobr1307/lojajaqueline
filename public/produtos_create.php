...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validação extra para código de barras duplicado
    if (!$erros) {
        $st = $db->prepare("SELECT 1 FROM produtos WHERE codigo_barras = :cb LIMIT 1");
        $st->execute([':cb' => $dados['codigo_barras']]);
        if ($st->fetch()) {
        ...
        <div class="w-100" style="max-width:700px;">
            ...
            <form method="post" class="card shadow p-4 bg-light">
                <div class="mb-3">
                    ...
                    <input type="text" name="codigo_barras" id="codigo_barras" class="form-control" required autofocus value="<?=htmlspecialchars($_POST['codigo_barras'] ?? '')?>">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="marca" id="marca" class="form-control" value="<?=htmlspecialchars($_POST['marca'] ?? '')?>">
                </div>
                <div class="mb-3">
                    <label class="form-label">Preço (R$)</label>
                    <input type="number" name="preco" class="form-control" step="0.01" min="0" required value="<?=htmlspecialchars($_POST['preco'] ?? '0.00')?>">
                </div>
                <div class="mb-3">
                    <label class="form-label">Quantidade</label>
                    <input type="number" name="quantidade" class="form-control" min="0" required value="<?=htmlspecialchars($_POST['quantidade'] ?? '0')?>">
                </div>
